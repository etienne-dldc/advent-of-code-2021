...
lines.forEach((line) => {
  ...
  // diagonal line
  const xStep = (line.x2 - line.x1) / Math.abs(line.x2 - line.x1);
  const yStep = (line.y2 - line.y1) / Math.abs(line.y2 - line.y1);
  console.log({ xStep, yStep });
  const dx = Math.abs(line.x2 - line.x1);
  for (let index = 0; index <= dx; index++) {
    gridAdd(line.x1 + index * xStep, line.y1 + index * yStep);
  }
});
